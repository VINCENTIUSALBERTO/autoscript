...
# ==============================================================================
# V2Ray VLESS Module
# Provides VLESS account management functions
# ==============================================================================
...
# VLESS accounts directory
VLESS_ACCOUNTS_DIR="${ACCOUNTS_DIR}/vless"
...
show_vless_menu() {
    while true; do
        clear_screen
        echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
        echo -e "${BCYAN}                    V2RAY VLESS MENU                           ${NC}"
        echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
        echo ""
        echo -e "  ${BGREEN}1.${NC} Create VLESS Account"
        echo -e "  ${BGREEN}2.${NC} View VLESS Accounts"
        echo -e "  ${BGREEN}3.${NC} Renew VLESS Account"
        echo -e "  ${BGREEN}4.${NC} Delete VLESS Account"
        echo -e "  ${BGREEN}5.${NC} Check VLESS Account Details"
        echo ""
...
create_vless_account() {
    ...
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BCYAN}                   CREATE VLESS ACCOUNT                        ${NC}"
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    while true; do
        ...
        if validate_username "$username"; then
            if [[ -f "${VLESS_ACCOUNTS_DIR}/${username}.json" ]]; then
    ...
    port="${V2RAY_PORT_VLESS:-8443}"
    path="${V2RAY_PATH_VLESS:-/vless}"
...
EOF
    ...
    echo "$account_data" > "${VLESS_ACCOUNTS_DIR}/${username}.json"
    
    # Generate VLESS link
    local vless_link="vless://${uuid}@${domain}:${port}?path=${path}&security=tls&encryption=none&type=ws#${username}"
    
    # Update V2Ray config (add user to inbound)
    add_vless_to_v2ray "$uuid" "$username"
    
    # Display result
    echo ""
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BGREEN}              VLESS ACCOUNT CREATED SUCCESSFULLY              ${NC}"
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    log_info "Created VLESS account: $username (expires: $exp_date)"
    
    press_any_key
}

# View all VLESS accounts
view_vless_accounts() {
    ensure_vless_dir
    clear_screen
    
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BCYAN}                    VLESS ACCOUNTS LIST                        ${NC}"
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    for file in "${VLESS_ACCOUNTS_DIR}"/*.json; do
        if [[ -f "$file" ]]; then
    ...
    if [[ $count -eq 0 ]]; then
        echo -e "  ${DIM}No VLESS accounts found${NC}"
    fi
...
renew_vless_account() {
    ...
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BCYAN}                    RENEW VLESS ACCOUNT                        ${NC}"
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    if [[ $count -eq 0 ]]; then
        print_error "No VLESS accounts found"
        press_any_key
        return
    fi
    ...
    local account_file="${VLESS_ACCOUNTS_DIR}/${username}.json"
    ...
    log_info "Renewed VLESS account: $username (new expiry: $new_expiry)"
...
delete_vless_account() {
    ...
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BCYAN}                    DELETE VLESS ACCOUNT                       ${NC}"
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    if confirm "Are you sure you want to delete account '$username'?"; then
        ...
        remove_vless_from_v2ray "$uuid"
        
        # Delete account file
        rm -f "$account_file"
        
        print_success "Account deleted successfully: $username"
        log_info "Deleted VLESS account: $username"
    else
...
# Check VLESS account details
check_vless_account() {
    ensure_vless_dir
    clear_screen
    
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${BCYAN}                  CHECK VLESS ACCOUNT DETAILS                  ${NC}"
    echo -e "${BWHITE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
...
# Add VLESS user to V2Ray configuration
add_vless_to_v2ray() {
    local uuid="$1"
    local email="$2"
    ...
    if [[ -s "$tmp_config" ]]; then
        ...
        log_info "Added VLESS user to V2Ray: $email"
    else
        rm -f "$tmp_config"
        log_error "Failed to add VLESS user to V2Ray config"
    fi
}

# Remove VLESS user from V2Ray configuration
remove_vless_from_v2ray() {
    local uuid="$1"
    local v2ray_config="${V2RAY_CONFIG_DIR}/config.json"
    ...
    # Remove user from vless inbound
    local tmp_config="${v2ray_config}.tmp"
    jq --arg uuid "$uuid" '
        .inbounds[] |= if .protocol == "vless" then
    ...
    if [[ -s "$tmp_config" ]]; then
        mv "$tmp_config" "$v2ray_config"
        restart_service "v2ray" 2>/dev/null || true
        log_info "Removed VLESS user from V2Ray: $uuid"
    else
        rm -f "$tmp_config"
    fi
}
...
# Export functions
export -f show_vless_menu create_vless_account view_vless_accounts
export -f renew_vless_account delete_vless_account check_vless_account
export -f add_vless_to_v2ray remove_vless_from_v2ray
